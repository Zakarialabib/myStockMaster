...
>
        ...
        <div class="flex items-start gap-2">
            <div class="hidden sm:block shrink-0 mt-1">
                @if ($item->attributes['image'] ?? null)
                    ...
                    />
                @else
                    <div class="w-10 h-10 bg-gray-100 dark:bg-gray-700 rounded-lg border border-gray-200 dark:border-gray-600 flex items-center justify-center text-gray-400">
            ...
            <div class="flex flex-col min-w-0">
                ...
                <div class="flex items-center gap-2 mt-1 flex-wrap">
                    <span class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-medium bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 border border-blue-100 dark:border-blue-800">
                        {{ $item->attributes['code'] ?? '' }}
                    </span>
                    <span class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-medium {{ ($item->attributes['stock'] ?? 0) > 0 ? 'bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-400 border border-green-100 dark:border-green-800' : 'bg-red-50 dark:bg-red-900/30 text-red-700 dark:text-red-400 border border-red-100 dark:border-red-800' }}">
            ...
            <div x-show="editPrice" @click.away="editPrice = false" x-transition>
                <div wire:change="updatePrice('{{ $item->rowId }}', '{{ $item->id }}')" class="flex flex-col">
                    <input
                        ...
                        wire:model.live.debounce.300ms="price.{{ $item->id }}"
        ...
        <div class="mt-0.5 inline-block">
            <div class="flex items-center justify-center gap-1">
                ...
                <input
                    ...
                    wire:model.live.debounce.300ms="quantity.{{ $item->id }}"
    ...
    <td class="px-3 py-3 align-top text-right">
        <div
            class="font-bold text-green-600 dark:text-green-400 mt-1 whitespace-nowrap text-sm"
            x-text="formatCurrency((parseFloat(prices[{{ $item->id }}]) || 0) * (parseFloat(quantities[{{ $item->id }}]) || 0))"
        >
