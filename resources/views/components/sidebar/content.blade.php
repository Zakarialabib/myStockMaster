<x-perfect-scrollbar as="nav" aria-label="main" wire:navigate:scroll class="flex flex-col flex-1 gap-1 px-3 py-4 md:gap-2 lg:gap-3">
    ...
    @can('user_access')
        <x-sidebar.dropdown title="{{ __('Notifications') }}" :active="request()->routeIs(['notifications.bell', 'notifications.manager'])" :icon="'<i class=\'fas fa-bell w-5 h-5\'></i>'">
            <x-sidebar.sublink title="{{ __('Notification Bell') }}" href="{{ route('notifications.bell') }}" :active="request()->routeIs('notifications.bell')" :icon="'<i class=\'fas fa-bell w-4 h-4\'></i>'" />
