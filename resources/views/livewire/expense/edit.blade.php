<div>
    <x-modal wire:model="editModal" name="editModal">
        ...
        <x-slot name="content">
            <form wire:submit="update" x-data="{ frequency: @entangle('form.frequency') }">
                <div class="flex flex-wrap mb-3">
                    <div class="md:w-1/2 sm:w-full px-3">
                        <x-label for="reference" :value="__('Reference')" />
                        <x-input wire:model="form.reference" id="reference" type="text" required />
                        <x-input-error :messages="$errors->get('form.reference')" class="mt-2" />
                    </div>
                    <div class="md:w-1/2 sm:w-full px-3">
                        <x-label for="date" :value="__('Date')" />
                        <x-input-date wire:model="form.date" id="date" name="date" required />
                    ...
                    <div class="md:w-1/2 sm:w-full px-3">
                        ...
                        <select required
                            ...
                            id="category_id" name="category_id" wire:model="form.category_id">
                    ...
                    <div class="md:w-1/2 sm:w-full px-3">
                        ...
                        <select
                            class="block w-full shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm border-gray-300 rounded-md mt-1"
                            id="warehouse_id" name="warehouse_id" wire:model="form.warehouse_id">
                            <option value="">{{ __('Select...') }}</option>
                            @foreach ($this->warehouses as $index => $warehouse)
                                <option value="{{ $index }}" @if ($form->warehouse_id == $index) selected @endif>
                                    {{ $warehouse }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="md:w-1/2 sm:w-full px-3">
                        ...
                        <x-input wire:model="form.amount" id="amount" type="text" required />
                    ...
                    <div class="w-full px-3 mb-4">
                        ...
                        <textarea
                            class="block w-full shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm border-gray-300 rounded-md mt-1"
                            rows="6" wire:model="form.description" id="description"></textarea>
