<div>
        ...
        <x-table.tbody>
            @forelse($categories as $category)
                <x-table.tr>
                    ...
                    <x-table.td>
                        <button wire:click="$emit('showModal', {{ $category->id }})" wire:loading.attr="disabled"
                            class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded">
                            <i class="fas fa-eye"></i>
                        </button>
                        <button wire:click="$emit('editModal', {{ $category->id }})" wire:loading.attr="disabled"
                            class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
                            <i class="fas fa-edit"></i>
                        </button>
                        <button wire:click="$emit('deleteModal', {{ $category->id }})" wire:loading.attr="disabled"
                            class="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded">
    ...
    <x-modal wire:model="showModal">
        ...
        <x-slot name="content">
            <div>
                <div class="mb-4">
                    <label for="category_code">{{ __('Category Code') }} <span class="text-red-500">*</span></label>
                    <input class="block w-full px-4 py-3 mb-2 text-sm placeholder-gray-500 bg-white border rounded"
                        type="text" name="category_code" wire:model.defer="category.category_code" disabled />
                </div>
                <div class="mb-4">
                    <label for="category_name">{{ __('Category Name') }} <span class="text-red-500">*</span></label>
                    <input class="block w-full px-4 py-3 mb-2 text-sm placeholder-gray-500 bg-white border rounded"
                        type="text" name="category_name" wire:model.defer="category.category_name" disabled />
                </div>
