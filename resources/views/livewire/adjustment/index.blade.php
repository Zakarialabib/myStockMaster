        ...
        <x-slot name="filters">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <x-input.text wire:model.live.debounce.500ms="search" placeholder="{{ __('Search adjustments...') }}" icon="fas fa-search" />
                </div>
            </div>
            @if ($selected)
        ...
        <x-table>
            ...
            <x-table.tbody>
                @forelse($adjustments as $adjustment)
                    <x-table.tr wire:loading.class.delay="opacity-50" class="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                        <x-table.td>
                            <div class="flex items-center">
                        ...
                        <x-table.td>
                            <div class="flex items-center space-x-2">
                                ...
                                @can('adjustment_update')
                                    <x-button href="{{ route('adjustments.edit', $adjustment) }}" variant="warning" size="sm" icon="fas fa-edit">
                                        <span class="sr-only">{{ __('Edit') }}</span>
                                    </x-button>
                                @endcan
                                @can('adjustment_delete')
                                    <x-button wire:click="deleteModal({{ $adjustment->id }})" variant="danger" size="sm" icon="fas fa-trash">
                    ...
                    <x-table.tr>
                        <x-table.td colspan="7">
                            <div class="flex flex-col items-center justify-center py-12">
                                ...
                                <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100 mb-2">{{ __('No adjustments found') }}</h3>
                                <p class="text-gray-500 dark:text-gray-400 text-center max-w-sm">
                                    {{ __('Get started by creating your first stock adjustment.') }}
                                </p>
                                @can('adjustment_create')
                                    <x-button href="{{ route('adjustments.create') }}" variant="primary" class="mt-4" icon="fas fa-plus">
                                        {{ __('Create Adjustment') }}
                                    </x-button>
                                @endcan
                            </div>
