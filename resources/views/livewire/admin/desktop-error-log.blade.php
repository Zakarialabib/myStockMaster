<div class="space-y-6">
        ...
        <div class="bg-white rounded-lg shadow-sm border p-6">
                        ...
                        @foreach ($statistics['by_category'] as $category => $count)
                            <div class="flex items-center justify-between">
                                <div class="flex items-center space-x-2">
                                    <x-heroicon-o-{{ $this->getCategoryIcon($category) }} class="w-4 h-4 text-gray-500" />
            ...
            @if ($statistics['most_frequent'])
                <div class="mt-6 p-4 bg-gray-50 rounded-lg">
                    ...
                    <p class="text-xs text-gray-500 mt-1">Occurred {{ $statistics['most_frequent']['count'] }} times</p>
                </div>
            @endif
        </div>
    @endif
    ...
    <div class="bg-white rounded-lg shadow-sm border p-4">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
            ...
            <div>
                ...
                <select wire:model.live="filterSeverity" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
                        ...
                        @foreach ($errorHistory as $error)
                            <tr class="hover:bg-gray-50">
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    ...
                                    <div class="text-xs text-gray-500">{{ $this->getRelativeTime($error['timestamp']) }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="px-2 py-1 rounded text-xs {{ $this->getSeverityColor($error['severity'] ?? 'medium') }}">
                                        {{ ucfirst($error['severity'] ?? 'medium') }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                    <div class="flex items-center space-x-2">
                                        <x-heroicon-o-{{ $this->getCategoryIcon($error['category'] ?? 'application') }} class="w-4 h-4 text-gray-500" />
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <button 
                                        wire:click="viewErrorDetails('{{ $error['id'] }}')"
    ...
    @if ($selectedError)
        <div 
            x-data="{ show: false }"
            x-show="show"
            x-on:show-error-modal.window="show = true"
            x-on:hide-error-modal.window="show = false"
            class="fixed inset-0 z-50 overflow-y-auto"
            style="display: none;"
        >
            <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                <div class="fixed inset-0 transition-opacity bg-gray-500 bg-opacity-75" x-on:click="$wire.closeErrorDetails()"></div>
                ...
                <div class="inline-block w-full max-w-4xl p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl rounded-lg">
                    <div class="flex items-center justify-between mb-4">
                        <h3 class="text-lg font-medium text-gray-900">{{ __('desktop.logging.view_error_details') }}</h3>
                        <button 
                            wire:click="closeErrorDetails"
                    ...
                    <div class="space-y-4">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-sm font-medium text-gray-700">{{ __('desktop.logging.error_timestamp') }}</label>
                                <p class="mt-1 text-sm text-gray-900">{{ $this->formatTimestamp($selectedError['timestamp']) }}</p>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700">{{ __('desktop.logging.error_severity') }}</label>
                                <span class="mt-1 inline-block px-2 py-1 rounded text-xs {{ $this->getSeverityColor($selectedError['severity'] ?? 'medium') }}">
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700">{{ __('desktop.logging.error_message') }}</label>
                        ...
                        @if (isset($selectedError['file']))
                            <div>
                                ...
                                <p class="mt-1 text-sm text-gray-900">{{ $selectedError['file'] }}:{{ $selectedError['line'] ?? '?' }}</p>
                            </div>
                        @endif
