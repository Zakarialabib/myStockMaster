...
<div>
    ...
    <div class="w-full relative mb-6 px-4 flex justify-center gap-4 items-center">
        ...
        <div class="flex-1">
            <input type="date" wire:model.live="startDate" class="w-full border rounded-sm px-2 py-1">
        </div>
        <span class="mx-2 font-semibold">{{ __('to') }}</span>
        <div class="flex-1">
            <input type="date" wire:model.live="endDate" class="w-full border rounded-sm px-2 py-1">
        </div>
    </div>
            ...
            @foreach (settings()->analytics_control as $control)
                @if ($control['status'])
                    @switch($control['name'])
                        ...
                        @case('sales')
                            <div class="sm:w-1/4 w-1/2 px-2 pb-2">
                                <x-counter-card color="{{ $control['color'] }}" counter="{{ $salesCount }}" :title="__('Sales')"
                        ...
                        @case('purchases')
                            <div class="sm:w-1/4 w-1/2 px-2 pb-2">
                                <x-counter-card color="{{ $control['color'] }}" counter="{{ $purchasesCount }}" :title="__('Purchases')"
                        ...
                        @case('average_payment_received_per_sale')
                            <div class="sm:w-1/4 w-1/2 px-2 pb-2">
                                <x-counter-card color="green" counter="{{ format_currency($average_payment_received_per_sale) }}"
