<div class="space-y-6">
    <div class="text-center">
        <h2 class="text-2xl font-bold text-gray-900">Demo Data Selection</h2>
        <p class="mt-2 text-gray-600">Choose a business line to populate your system with relevant demo data</p>
    </div>
    ...
    <div class="space-y-4">
        ...
        <div class="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
            <div>
                <h3 class="text-lg font-medium text-gray-900">Install Demo Data</h3>
                <p class="text-sm text-gray-600">Populate your system with sample products, categories, and data</p>
            </div>
            <label class="relative inline-flex items-center cursor-pointer">
                <input type="checkbox" wire:model="install_demo_data" class="sr-only peer">
        ...
        @if($install_demo_data)
            ...
            <div class="space-y-4">
                <h3 class="text-lg font-medium text-gray-900">Select Business Line</h3>
                    ...
                    @foreach($businessLines as $key => $line)
                        <div class="relative">
                            <input type="radio" 
                                   ...
                                   name="business_line" 
                                   value="{{ $key }}" 
                                   wire:model="selected_business_line"
                                   class="sr-only peer">
                            <label for="business_{{ $key }}" 
                                   class="flex flex-col items-center p-4 bg-white border-2 border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50 peer-checked:border-blue-500 peer-checked:bg-blue-50 transition-all">
                                <div class="text-3xl mb-2">{{ $line['icon'] }}</div>
                                <div class="text-sm font-medium text-gray-900 text-center">{{ $line['name'] }}</div>
                                <div class="text-xs text-gray-500 text-center mt-1">{{ $line['description'] }}</div>
                            </label>
                        </div>
                    @endforeach
                </div>
            ...
            @if($selected_business_line)
                <div class="p-4 bg-blue-50 border border-blue-200 rounded-lg">
                    <h4 class="font-medium text-blue-900">Selected: {{ $businessLines[$selected_business_line]['name'] }}</h4>
